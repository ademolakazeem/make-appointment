...
def get_duration(appointment_type):
    appointment_duration_map = {'cleaning': 30,
                                'root canal': 60, 'whitening': 30}
...
def build_options(slot, appointment_type, date, booking_map):
    ...
    if slot == 'AppointmentType':
        ...
        for i in range(min(len(availabilities), 5)):
            options.append({'text': build_time_output_string(
                availabilities[i]), 'value': build_time_output_string(availabilities[i])})
...
def make_appointment(intent_request):
    ...
    output_session_attributes.get('id', None)
    if (output_session_attributes.get('id') == None):
    ...
    if source == 'DialogCodeHook':
        ...
        if not appointment_type:
            return elicit_slot(
                ...
                build_response_card(
                    ...
                    build_options('AppointmentType',
                                  appointment_type, date, None)
                )
            )
        ...
        if appointment_type and not date:
            return elicit_slot(
                ...
                intent_request['currentIntent']['slots'],
                'Date',
                {'contentType': 'PlainText', 'content': 'When would you like to schedule your {}?'.format(
                    appointment_type)},
                build_response_card(
                    'Specify Date',
                    'When would you like to schedule your {}?'.format(
                        appointment_type),
        ...
        if appointment_type and date:
            ...
            if booking_availabilities is None:
                ...
                output_session_attributes['bookingMap'] = json.dumps(
                    booking_map)
            ...
            if len(appointment_type_availabilities) == 0:
                ...
                return elicit_slot(
                    ...
                    build_response_card(
                        ...
                        build_options('Date', appointment_type,
                                      date, booking_map)
                    )
                )
                ...
                return confirm_intent(
                    ...
                    slots,
                    {
                        ...
                        'content': '{}{} is our only availability, does that work for you?'.format
                                   (message_content, build_time_output_string(
                                       appointment_type_availabilities[0]))
                    },
                    build_response_card(
                        'Confirm Appointment',
                        'Is {} on {} okay?'.format(build_time_output_string(
                            appointment_type_availabilities[0]), date),
                        [{'text': 'yes', 'value': 'yes'},
                            {'text': 'no', 'value': 'no'}]
                    )
                )
            ...
            return elicit_slot(
                ...
                slots,
                'Time',
                {'contentType': 'PlainText', 'content': '{}{}'.format(
                    message_content, available_time_string)},
                build_response_card(
                    'Specify Time',
    ...
    if booking_availabilities:
        ...
        if duration == 60:
            second_half_hour_time = increment_time_by_thirty_mins(
                appointment_time)
